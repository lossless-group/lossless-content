...
tags: [Callouts, Blockquote-Syntax, Directive-Migration, Markdown-Processing, Component-Architecture, ArticleCallout, Syntax-Standardization, TypeScript-Fixes]
---
...
const calloutTypes = {
  ...
  'success': { icon: '✅', title: 'Success', color: '#4ade80', bgColor: 'rgba(34, 197, 94, 0.08)', borderColor: 'rgba(34, 197, 94, 0.3)' },
...
- `LLMResponse.astro` - For AI/LLM generated content with robot icon 🤖
...
- `Note.astro` - Clean note styling with document icon 📝
- `Success.astro` - Success confirmation styling with green theme ✅
- `Error.astro` - Error alert styling with red theme ❌
- `Tip.astro` - Helpful tip styling with purple theme 💡
- `Emphasis.astro` - Special emphasis styling with enhanced visual impact 💡
...
├── components/
│   └── markdown/
│       ├── AstroMarkdown.astro (modified - removed 200+ lines of directive logic)
│       └── callouts/
│           ├── ArticleCallout.astro (enhanced - [!Type] pipeline with component routing)
│           └── styled/
│               ├── LLMResponse.astro (existing - AI/LLM content styling)
│               ├── Warning.astro (new - enhanced warning callouts)
│               ├── Info.astro (new - professional information callouts)
│               ├── Note.astro (new - clean documentation notes)
│               ├── Success.astro (new - success confirmation styling)
│               ├── Error.astro (new - error alert styling)
│               ├── Tip.astro (new - helpful tip styling)
│               ├── Emphasis.astro (new - special emphasis styling)
│               └── Quote.astro (new - elegant quote styling)
└── utils/
    └── markdown/
        └── remark-directives.ts (modified - removed callout directive mappings)

Migration Summary:
├── Removed: 8 callout directive mappings from remark-directives.ts
├── Removed: 200+ lines of callout processing logic from AstroMarkdown.astro
├── Enhanced: ArticleCallout.astro with [!Type] syntax detection and component routing
├── Added: 8 new styled components (Warning, Info, Note, Success, Error, Tip, Emphasis, Quote)
├── Implemented: Complete Obsidian-style callout pipeline with graceful fallbacks
├── Fixed: 2 TypeScript linter errors related to array type checking
